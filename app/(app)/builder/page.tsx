...
export default function BuilderPage() {
  const { selectedSector, addActivity, addToQueue } = useApp();
  ...
  const handleStartBuild = async () => {
    ...
    setBlueprint([]);
    addLocalLog(`Initiating Neural Scan for ${industry}...`);
    addActivity(`Neural Architect started for ${industry}`, 'builder');
    
    setTimeout(() => addLocalLog(`Analyzing Strategic Intent: "${prompt || 'Default Optimization'}"...`), 500);
    setTimeout(() => addLocalLog(`Generating Component Matrix (Style: ${style})...`), 1000);
    ...
    try {
        ...
        const generatedBlueprint: Component[] = [
            ...
            { id: '2', type: 'SUBTEXT', title: (data.sub || 'Strategic Mission').substring(0, 40) + '...', status: 'draft' },
        ...
        toast("Architecture Complete: Ready for sync.", "success");
    } catch (error) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-black/80 backdrop-blur-xl animate-in fade-in duration-500">
            <div className="w-full max-w-7xl h-full max-h-[90vh] bg-[#050506] border border-white/10 rounded-[2.5rem] overflow-hidden flex flex-col relative shadow-[0_0_100px_rgba(0,255,209,0.1)]">
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide relative bg-[radial-gradient(circle_at_50%_0%,rgba(0,255,209,0.15)_0%,transparent_50%)]">
                    ...
                    {/* Hero Section */}
                    <div className="min-h-[80vh] flex flex-col items-center justify-center text-center px-8 relative pt-20">
                        <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff05_1px,transparent_1px),linear-gradient(to_bottom,#ffffff05_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)]" />
                        <div className="p-3 bg-[#00ffd1]/10 border border-[#00ffd1]/20 rounded-full mb-8 text-[#00ffd1] text-[10px] font-mono tracking-[0.3em] uppercase animate-pulse">
                            Generated by AIVA OS
                        </div>
